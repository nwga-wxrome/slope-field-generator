...
class CustomDialog(simpledialog.Dialog):
    """Custom dialog that ensures it stays on top"""
    ...
    def body(self, master):
        self.lift()  # Lift the dialog above other windows
...
class SlopeFieldApp:
    def __init__(self, root):
        ...
        self.function_string = "-cos(x)"  # Default function
        self.dark_mode = False

        # Make sure the root window stays on top initially
        self.root.lift()
        ...
        # Set the window icon
        try:
            icon_path = resource_path("slope-field-1.png")
    ...
    def prompt_for_function(self):
        dialog = CustomDialog(
            self.root,
            "Input",
            "Enter new function f(x, y):",
            self.function_string
        )
        if dialog.result:
            self.function_string = dialog.result
            self.plot_slope_field()
